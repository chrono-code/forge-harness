...
プロジェクトを独立した特化ハーネスとして**送り出します (EMIT)**。これが目指す目標です。

### 実際に手に入るもの — 5つの正体 (five identities)

これは5つのモジュールではありません。スキルが**固まっていく形**の名前です — 上に載せた新しい層では
なく、スキルとエージェントに散らばって既にそこにあったものに名前を付けたものです。
...
成熟度は正体ごとに4段階（`ideal → partial → RC (ラボで立った) → REALIZED (外を歩いた)`）で追跡され、
その等級は意図的にただ1箇所にだけ置かれています:
[`ship_readiness_gate.md`](knowledge/shared/harness-core/ship_readiness_gate.md)。ここには**あえて
写しません** — 2つのファイルに置かれた等級は片方が必ず腐るからで、このレポは自分自身でそれを複数回
測っています。
...
- **フロンティアに継ぎを当てるのではなく、フロンティアに乗ります。** FH はファミリーをまたいで
  ...
  *ありません*。そうしたスキャフォールディングはモデルが強くなれば死ぬからです。これは共進化です:
  substrate がいまやネイティブでやってくれるものは脱ぎ捨て、次に出してくるものは吸収します。脱相関
  (decorrelation) は*いまの*信頼の梃子であり、クロスファミリーのパネルは単一モデルの天井を超えます。
...
**4大エンジン。** それぞれが上のいずれかの正体を足元で支えています。ゲートの表は既にこの4つを機械的に
持っていました — 名前を付けたのは発明ではなく認識です。
...
`judgment-circuit` はもっとも誤読されやすいので、はっきり書きます: これは**アイデンティティの宣言では
ありません**。日本語で「魂」のような1語に言い換えないでください — その語は*ペルソナ*として読まれますが、
このエンジンの背後にある105ランの測定の最大の発見は、まさに**ペルソナ宣言は判断回路ではない**という
ことでした:「あなたは〜だ」は弱いティアで*正味の損失*として測定され、それを取り除くと **+0.67** 回復
しました。1語の言い換えは、測定が切り分けたものをそのまま再融合させてしまいます。また判断回路は一度に
作られるものでもありません — FH は新しいハーネスに**種となる草案**を渡し、そのハーネスが実際に使われる
なかで埋まっていきます。
...
② 中間では脱相関して      軸を選んで一度に当てます。軸は掛け算するのではなく「選ぶ」— 並列化それ自体には
   加速する               方向がなく、方向を与えるのは判断回路です
...
| 軸 | この軸が発火したとき何が間違っているか | 典型的な計器 |
|---|---|---|
| **ⓐ 別ファミリー** | **実装**が間違っている | クロスファミリーの敵対的レビュー (`auto-decorrelation`) |
| **ⓑ 初の実使用** | **測り方**が間違っている | 実際の対象に対して一度走らせる |
| **ⓒ 記録のグラウンディング** | **主張**が間違っている | 隔離された監査が文書の数値と引用を測り直す |
| **ⓓ 戻して観察する** | **アンカー**が間違っている | 配線を消して、対応するレーンが本当に赤くなるか確かめる |

5つ目の軸 — **standpoint（立ち位置）** — は変更が別のハーネスへまたがるときに適用されます:
自分の読み方ではなく、*対象側*のレポとルールから diff を走らせてください
([`field_verdict_crossfamily_gate.md §7`](knowledge/shared/harness-core/field_verdict_crossfamily_gate.md))。

4つを毎回すべて回すわけではありません。自分がいま実際にさらされている失敗モードに合う軸を選びます。
このコスト境界は意図的です — 1行の修正はどれも要求せず、verdict/ゲートの変更は ⓐ を要求し、不可逆な
表面はそれ以上を要求します。
