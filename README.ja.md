...
します。番号付きのメニューが出て、そこからはツールが案内します。入口を選んでいくつか答えれば、
インストールウィザードまで代わりに実行します。

<sub>🟥 <b>いまの最後の部分について正直に</b>: 言語を合わせることは散文のルールであって、その背後に
機械的なものは何もなく、いつも守られるわけではありません。2026-08-21 にフロアティアでブラインド計測
したところ、<b>あなたがいま作ったのと同じ新しいクローン</b>では、中国語も韓国語も、ドアのラベルまで
含めてまるごとその言語で返ってきました。一方、既定の言語がピン留めされている開発者本人のマシンでは、
中国語は<b>5回に1回</b>しか当たりませんでした — この注記が存在する理由はそれです。まだ揺れているのは
メニューが出るかどうかで、ある挨拶のかたちではメニューが出ませんでした。違う言語で返ってきたときも、
メニューが飛ばされたときも、そう言ってください — すぐ切り替えます。この残余はならして隠さず
<code>CLAUDE.md</code> §Voice/Tone に書いてあります。</sub>

この線から下は必要になったときに見る参考資料であり、始める前の宿題ではありません。

- **増幅するもの** — 試行の回数です。試行錯誤があなたから離れ、並列で回ります。

- **増幅しないもの** — モデルの天井です。ハーネスはモデルを自分の天井まで引き上げるだけで、
  その先へ押し上げることはできません。

- **確かめ方** — 自分の評価を公開します。**自分が何であると主張しているか**を5つの名前で挙げておき
  （ハーネスクラスター · プロジェクトインキュベーター · ガバナンスゲート · フロンティア吸収 · 増幅器）、その一つひとつを
  [リリース](https://github.com/chrono-meta/forge-harness/releases)ごとに正直に採点します。緑でない欄は、
  まだ足りていない実際の実行が何かを名前で述べます。
  <sub>5つそれぞれは、この下の «5つの正体» の節で詳しく書いています。</sub>
...
1. セットアップがうまくいったことは、挨拶（どの言語でも）で 🐿️ のドアメニューが出て、
   「プロジェクトを接続して」で `tracks/{your-project}/` ができることで分かります。
...
[`ship_readiness_gate.md`](knowledge/shared/harness-core/ship_readiness_gate.md) — アイデンティティ
ごとの現在の等級が、日付とその等級を得た根拠とともに書かれています。すべてが緑ではありません。
どれがいつなのかは、そのファイルが語ります。
...
| `external-grounding` | 新規性を主張したり設計を決めたりする*前に*、レポの外へ手を伸ばすこと | ④ フロンティア吸収 |
...
**速度は最後の矢印であって、4つめの箱ではありません。** そして ② は**別々に回す2つのダイヤル**です
— *脱相関*（盲点のリスクに反応します: 別のモデルファミリー ⓐ、別の立ち位置 ⓑ）と*並列化*（表面の
大きさに反応します: 1つの文脈に収まるか）。掛け算せずに、選んでください。

```
① 設計の前に回路を        判断回路が最初に入ります — 成功 · 傾け方 · 範囲外 · 決してやらないこと —
                          やったことの記録として後から書き起こすものではありません

② 並列脱相関              作業を「違うかたちで失敗する」検査に分けて、一度に走らせます。どの違いが効くかを
   中間で、加速する       選んでください — 同じ種類のレビュアーをもう1人足すのは脱相関ではなく、同じ盲点を
...
> 📖 **ここから先は、もっと知りたい方のためのものです — 使い始めるのに必要ではありません。**
> インストールして動かしに来たのなら、いちばん上の「2分」の節がすべてです。ここでいったん止めて、
> どれかの検査に「おや?」と思ったときに戻ってきてください。この先はゲートがなぜその形をしているのか
> という話で、すでにハブを回している人に向けて書かれています。見慣れない言葉は →
...
| **ⓑ 立ち位置 (standpoint)** | 変更分 + **対象ハーネス自身の正典** | **引用した規約が本当にそう言っているか** | そのハーネス自身のレポ · ルールの側で変更分を走らせる ([`§7`](knowledge/shared/harness-core/field_verdict_crossfamily_gate.md)) |
| **ⓒ 隔離されたグラウンディング** | 著者が書いた文 — その主張、*そして* **始める前に宣言しておいたもの** — + いまのツリー | **主張**が間違っている · デルタが宣言した内容と食い違う | 書いていない誰かが、書かれている内容を測り直す。事前宣言の側は、書き留めた成功の定義をデルタに突き合わせて読み返すゲート |
...
> **ⓒ は 2026-08-21 に広がりました。そして「どう広がったか」のほうが役に立ちます。** このレポの
> コミットマーカーは 2026-08-09 以降、著者自身の**事前宣言** — *何を成功とみなすか* · *何は絶対に
> しないか* — を設計の*前に*書くことが義務づけられていました。その日コントロールを付けて測ってみると、
> **それを読むコードは0行**でした。兄弟フィールドが21か所で検査されている一方で、ゲートの仕様は
> そのフィールドの名前すら挙げていませんでした。実際のコーパスでは、**マーカー98件のうち37件が
> その行をまったく持って**おらず、その中にはレーン28本を通し他のフィールドをすべて埋めたパネル
> レビュー済みのものまでありました。軸はどれも*外側*を見ていました — 変更分、対象のレポ、先行事例、
> 成果物。**その記録自身の必須フィールド**を見た軸はひとつもありません。消費先のないスロットは
> つねに「できています」と報告します。**存在が判定の代わりをしてしまうから**です。
>
> 修理は7つめの軸ではありませんでした。ⓒ はすでに*著者が書いた文 + いまのツリー*を受け取っており、
> それは事前宣言の検査が受け取るものと**一字一句同じ**です。時制（先に宣言したのか、後から主張したのか）
> は敵対性と同じく**姿勢**であって、軸ではありません。新しく1本立てていたら、上のブラインド再分類が
> 見つけたのとまったく同じ誤りを繰り返すところでした。

> **ピアセッションは ⓑ と ⓓ に記録します — 2026-08-21 の決定であり、「どのピアに訊くか」が肝です。**
> 同じハーネスから分かれ、作業の**別の枝で熱く回っている**並列セッションは、あなたの複製ではありません。
> 熱くなったその地点で、そのセッションは実際に**第二の顔**を持ちます — 本物の立ち位置 (ⓑ) であり、
> 本物の他人のコードベース (ⓓ) です。ですからピアの判定はその2つの軸に記録し、7つめの軸は立てません
> でした。マーカーの `axes-run` の並びも変わっていません。
>
> 上の ⓓ の行が「別の会社のレポ」ではなく「**他人のコードベース**」と書かれているのは、これが理由です
> — 効いてくる境界は**その判定を生んだ作業の文脈が誰のものか**であって、ファイルを所有する GitHub の
> organization が誰のものかではありません。別の枝で熱くなっているピアはその境界の**外側**にいます。
> 逆に、この文脈から自分で起動したサブエージェントは、どれだけ違うレポを読ませても境界の**内側**です。
>
> 🟥 効いてくるのは系のほうです。**ピアには、そのピアが実際に熱くなった軸を訊いてください。** それ以外の
> ところでピアはあなたと同じ顔をしており、訊いても脱相関しません — 同じ入力、同じ盲点です。サブエージェント
> でも、自分の書いたものを読み直すことでも**代わりにはなりません。** 第二の顔は、そのセッションが実際に
> 別の作業を*したこと*から生まれるのであって、プロンプトで作れるものではありません。
...
🟥 **正直な際どさ**: エージェントが**ツールで自分から入力を取りに行く**と
...
## 鍛冶場 (The forge) — 名前はどこから来たか

forge-harness はプロジェクトを鋼のように扱い、この比喩は装飾ではなく文字通りです。
作業は形を与えられ、攻撃で硬くなり、そうして生き延びたからこそ、はじめてより速く出荷されます。

> 🟥 **5つめの番号付きの集合ではなく、3段工程を鍛冶屋の言葉で言い直したものです。**
> この節の以前の版は *「そのどの数字とも対応しません」* と書いていましたが、**それは誤りでした。**
> 二つの図がそれを露わにします — どちらも三段階と矢印一本、そして **⟹ 加速** で終わります。
> 既に同じ形をしていて、名前だけが離れていました。つまりこれは **一つの層を二度語ったもの**であり、
> 二つめの層ではありません。4大エンジン · 4軸ゲート · 6軸検証は依然として別の数字で、ここには対応しません。

**金床の上で三つ — そしてそれぞれが3段工程の一段階です:**

| 鍛冶屋の言葉 | 段階 | ここでは何を指すか | コマンド |
|---|---|---|---|
| **鍛え (Forge)** | ① **魂を植える** | 生のプロジェクトをハーネスへ形づくり、**設計の前に**「何を成功と呼ぶか / 決してやらないこと」を定める | `install-wizard` · `deep-clarify` · マーカーの必須 `①영혼` 行（コミットで強制） |
| **焼き入れ (Quench)** | ② **並列・脱相関の加速** | 急冷 — 脱相関した角度から多数の試行を並列に。速く硬くなり、🟥 **その代償に脆さが生まれます。** 鋼に焼き入れがすることが、まさにそれです | `auto-decorrelation` · `agent-composer` · `meta-prompt-builder` · 他系列サイドカー(codex · agy · gemini) · 隔離ワークツリー・レーン |
| **焼き戻し (Temper)** | ③ **6軸で焼き切る** | 再加熱して硬さは保ったまま脆さだけを抜く — **攻撃するパスが本当に属する場所** | `steel-quench` · `phantom-quench` · `sim-conductor` · `prompt-regression` · `verify-bidirectional` · `fh-meta:challenger` · 巻き戻しプローブ · `templates/temper_check.sh` |

> ⚠️ **スキル名は以前の読みを抱えたまま、そのままにしてあります。** `steel-quench` ·
> `phantom-quench` は名前こそ *焼き入れ* ですが、やっていること — 硬くなった資産を攻撃して脆さを
> 露わにすること — は **焼き戻し** です。比喩を合わせるために出荷済みのスキル名を変えるほうが
...
**⟹ そして、速くなります。** 鍛冶場を生き延びた刃はより速く斬る — `goal-quench`、
*Pass → Accelerate*。速度は上の三つが**生み出すもの**であって、四つめにやることではありません。
これはこのページ冒頭のタグラインを鍛冶屋の言葉で言い直したものと同じです:
品質が梃子であり、速度はその結果です。

上で名前を挙げたコマンドはすべて、いま出荷されています。焼き戻し (Temper) は作られる*前に*名前が
先に付けられ — 意図的に（参照: [`ETHOS.md`](docs/ETHOS.md#the-forge)）— 測定実行が検証したのちに
出荷されました。

鍛冶場の周りで、さらに2つのシグネチャが回り続けます: `harvest-loop`（各セッションの教訓が恒久
スキルになる）と `agent-composer`（ディスパッチをオーケストレーション）。残りのスキルは必要に
なるまで待ちます — 全リストは下に。
