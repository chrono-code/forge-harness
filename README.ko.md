...
다섯 개의 모듈이 아닙니다. 스킬들이 **뭉쳐서 만들어내는 형태**들이며, 스킬과 에이전트 위에 새로
얹은 층이 아니라 이미 그 안에 흩어져 있던 것에 이름을 붙인 것입니다.
...
성숙도는 정체성별로 `이상 → 부분 → RC(실험실에서 섬) → REALIZED(밖에서 걸음)` 4단계로 추적하며,
그 등급은 의도적으로 딱 한 곳에만 둡니다:
[`ship_readiness_gate.md`](knowledge/shared/harness-core/ship_readiness_gate.md). 여기에는 **옮겨
적지 않습니다**. 두 파일에 나눠 둔 등급은 한쪽이 반드시 낡으며, 이 레포는 그것을 자기 자신에게서
여러 번 실측했습니다.
...
- **프런티어를 메꾸는 게 아니라 함께 탑니다.** FH는 여러 패밀리(Claude, Codex, Gemini, 로컬)에
  ...
  내놓는 것은 흡수합니다. 탈상관(decorrelation)은 *지금의* 신뢰 지렛대이고, 크로스패밀리 패널이
  단일 모델의 천장을 넘어섭니다.
...
**네 엔진.** 각각은 위의 어떤 정체성이 딛고 서 있는 바닥입니다. 게이트 표가 이미 이 넷을 기계적으로
지고 있었고, 이름을 붙인 것은 발명이 아니라 인식이었습니다.
...
가장 자주 오독되는 것이 `judgment-circuit`이라 못 박아 둡니다: 이것은 **정체성 선언이 아닙니다**.
영혼(심지)이라는 운영자의 말은 판단의 **좌표계**를 가리키는 것이지 페르소나가 아니며, 영어로
"soul"이라 옮기지 않는 이유도 그것입니다. 이 엔진 뒤에 있는 105회 실행 측정의 가장 큰 발견이
바로 **페르소나 선언은 판단 회로가 아니다**였습니다: "너는 ~이다"는 약한 티어에서 *순손실*로
측정됐고, 그것을 제거하자 **+0.67**이 회복됐습니다. 한 단어로 뭉뚱그리는 순간, 측정이 갈라놓은
것이 다시 붙습니다. 또한 판단 회로는 한 번에 만들어지지 않습니다 — FH는 새 하네스에 **씨앗
초안**을 쥐어주고, 그 하네스가 실제로 쓰이면서 채워집니다.
...
② 중간은 탈상관으로     축을 골라 한꺼번에 칠 것. 곱하지 말고 고를 것 —
   가속                 병렬화 자체엔 방향이 없고, 방향은 판단 회로가 줌

③ 끝에서 4축으로       아래 네 축. 적대검증은 그중 하나이지 전부가 아님
   태우기
```

**네 검증 축** — "리뷰했다"가 실제로는 첫 번째 축 하나만 뜻하는 경우가 대부분입니다:

| 축 | 이 축이 걸리면 무엇이 틀린 것인가 | 대표 계기 |
|---|---|---|
| **ⓐ 다른 패밀리** | **구현**이 틀림 | 크로스패밀리 적대검증(`auto-decorrelation`) |
| **ⓑ 첫 실사용** | **재는 방식**이 틀림 | 진짜 대상에 한 번 돌려보기 |
| **ⓒ 기록 그라운딩** | **주장**이 틀림 | 격리된 감사가 문서의 수치와 인용을 다시 잼 |
| **ⓓ 되돌려 관찰** | **앵커**가 틀림 | 배선을 지우고 해당 레인이 실제로 빨개지는지 확인 |

다섯 번째 축 — **관점(standpoint)** — 은 변경이 다른 하네스로 넘어갈 때 적용됩니다: 그 diff를
당신의 해석이 아니라 *대상 하네스의* 레포와 규칙에서 돌립니다
([`field_verdict_crossfamily_gate.md §7`](knowledge/shared/harness-core/field_verdict_crossfamily_gate.md)).

넷을 매번 다 돌리지는 않습니다. 실제로 노출된 실패 모드에 맞는 축을 고릅니다. 비용 경계는
의도적입니다 — 한 줄짜리 수정은 어느 축도 벌지 못하고, 판정·게이트 변경은 ⓐ를 벌며, 비가역
표면은 그보다 더 법니다.
