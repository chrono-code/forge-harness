...
위까지가 결정의 전부입니다. 아래는 필요할 때 찾아보는 자료입니다.

### ②의 네 줄을 친 다음

**`안녕` 이라고 입력하세요.** `hi` · `こんにちは` · `你好`, 편한 말이면 무엇이든 됩니다.
...
규칙</b>이라 늘 지켜지지는 않습니다. 2026-08-21 블라인드 플로어 티어로, <b>방금 하신 것과 같은
새 클론</b>에서 재 봤습니다. 중국어·한국어 인사 모두 <b>문 라벨까지 그 언어로</b> 돌아왔습니다.
반면 기본 언어가 핀으로 박힌 개발자 본인 머신에서는 중국어가 <b>5번 중 1번</b>만 맞았고, 이 각주가
있는 이유가 그것입니다. 아직 흔들리는 쪽은 <b>메뉴가 뜨는지</b>입니다. 인사말 한 종류에서는 메뉴가
안 떴습니다. 언어가 어긋나거나 메뉴가 안 뜨면 그냥 말씀하세요, 바로 바꿉니다. 이 잔여는 덮지 않고
<code>CLAUDE.md</code> §Voice/Tone 에 적어 두었습니다.</sub>

이 아래는 필요할 때 찾아보는 참고 자료이고, 시작하기 전에 읽어야 하는 숙제가 아닙니다.

- **증폭하는 것**: 시도 횟수입니다. 시행착오가 손을 떠나 병렬로 돕니다.

- **증폭하지 않는 것**: 모델의 천장입니다. 하네스는 모델을 자기 천장까지 끌어올릴 뿐,
  그 너머로 밀어 올리지는 못합니다.

- **확인하는 법**: 자기 등급을 공개합니다. **스스로 무엇이라고 주장하는지를 다섯 가지로
  이름 붙여 두고**(하네스 클러스터 · 프로젝트 인큐베이터 · 거버넌스 게이트 · 프런티어 답습 ·
  증폭자), 그 각각을 [릴리스](https://github.com/chrono-meta/forge-harness/releases)마다 정직하게
  채점합니다. 초록이 아닌 칸은 아직 없는 실제 실행이 무엇인지 이름으로 말합니다.
  <sub>다섯 각각은 아래 '다섯 정체성' 절에서 풀어 씁니다.</sub>
...
> **이 문서는 사람이 읽는 문서입니다.** AI 운영 규칙 → `CLAUDE.md` · 명령어 레퍼런스 → `CHEATSHEET.md`

**두 문에 붙는 각주 둘**입니다. 세 번째 표를 만들지 않습니다.

- **②를 프로젝트 하나에만 써보고 싶다면**
  [`templates/starter_profile.md`](templates/starter_profile.md): 명령 하나에 엄선한 첫 5개 스킬.
- **①에서 `npx` 대신 `brew`를 쓰고 싶다면**
  `brew tap chrono-meta/forge-harness && brew install forge-harness`. 내용은 100% 같고 설치 UX만
  다릅니다. 커뮤니티 탭이라 Homebrew Core에는 없고, 탭을 먼저 추가하지 않으면 `brew search` 로는
  안 잡힙니다.
...
**②에는 Claude Code CLI가 필요합니다**(`claude --version` 으로 확인). **①에는 필요 없고**, 그것이
①이 따로 있는 이유입니다.

<details><summary><b>선택: 게이트 하나에 Python + PyYAML이 필요합니다</b>. 없으면 <code>npm test</code>가 빨갛습니다</summary>
...
> 이미 다른 곳에 클론했나요? 그 경로가 허브입니다. 문서의 모든 `~/projects/forge-harness`를
> 실제 클론 경로로 바꿔 읽으세요.
...
이 페이지 맨 위의 문제 표와는 층이 다릅니다: 그 표는 *여기까지 들고 왔을 법한 증상*이고, 이것은
...
| **④** | **프런티어 답습** | 밖에서 들어온 것이 조직 *안쪽까지* 내려앉음 |
...
| `external-grounding` | net-new를 주장하거나 설계를 확정하기 *전에* 레포 밖에 손을 뻗는 것 | ④ 프런티어 답습 |
...
|---|---|---|---|
| **ⓐ 다른<br>패밀리** | 변경분 + 저자의 프레이밍 | **구현**이 틀림 | 다른 모델 패밀리의 리뷰어(`auto-decorrelation`) |
...
| **ⓒ 격리<br>그라운딩** | 저자가 쓴 문장 · 주장 *그리고* **시작 전에 선언해 둔 것**: + 지금의 트리 | **주장**이 틀림 · 델타가 선언한 것과 안 맞음 | 그것을 쓰지 않은 쪽이 적힌 내용을 다시 잼. 사전선언 쪽은, 적어 둔 성공 정의를 델타에 되짚는 게이트 |
| **ⓓ 3자<br>대면** | 문제 + **남의 코드베이스** | **이미 풀린 문제 아닌가** · 내 변경이 남의 레포를 어디서 만지나 | 관련 없는 제3의 레포에서 같은 문제를 봄 |
| **ⓔ 첫<br>실사용** | 실물 대상 한 건 | **재는 방식**이 틀림 · 계기의 계기 | 진짜 대상 하나에 돌리고 결과를 손으로 확인 |
| **ⓕ 되돌려<br>관찰** | 배선을 지운 트리 | **앵커**가 틀림 · 검사가 장식임 | 지키는 대상을 지우고 *바로 그* 검사가 빨개지는지 확인 |

> **ⓒ가 2026-08-21 에 넓어졌고, '어떻게' 넓어졌는지가 더 쓸모 있습니다.** 이 저장소의 커밋 마커는
> 2026-08-09 부터 저자 자신의 **사전선언**: *무엇을 성공으로 치나* · *무엇은 절대 안 하나*. 이 둘을
...
> 28개에 다른 필드는 전부 채운 패널-리뷰 통과본도 있었습니다. 여섯 축 전부 *바깥*을 봤습니다.
...
> 같은 하네스에서 갈라져 나와 **다른 갈래에서 뜨겁게 돌고 있는** 병렬 세션은 내 사본이
...
> 피어는 나와 같은 얼굴을 하고 있고, 물어도 탈상관이 안 됩니다. 같은 입력, 같은 사각.
...
> 재료**로 되어 있어서, 아래층이 위층을 씁니다. 모순은 *주어*에서 풀립니다: **엔진**은 FH가 사용자의
...
AI와 긴 공동 저작 세션을 거치고 나면, 나와 AI는 같은 맥락을 공유합니다. 그리고 같은 맹점도
공유합니다. 가질 가치가 있는 리뷰어는 내 추론을 한 번도 본 적 없는 리뷰어입니다. 손으로도
얻을 수 있습니다: 작업을 비어 있는 새 채팅에 붙여넣으면 됩니다. FH는 그 귀찮은 일을 명령 하나의
루틴으로 바꿔줄 뿐입니다.

- **사이드카 / 에이전트 디스패치** → 내 세션의 맥락이 전혀 없는 리뷰어
- **steel-quench · phantom-quench** → 그 냉정한 검토를, 필요할 때 바로

모델에 구애받지 않습니다: 한 AI와 함께 만들고, 냉정한 검토는 다른 어떤 AI로든 돌립니다. 원래
세션에 없었던 쪽이 내 냉정한 리뷰어입니다. 이것은 모델 순위 매기기가 아닙니다.

**FH가 주장하지 않는 것:** 냉정한 검토는 쓰고 있는 베이스 모델 자체의 능력이지, FH가 추가하는 탐지
...
> 🟥 **다섯 번째 번호 붙은 집합이 아니라, 3단 공정을 대장장이의 말로 옮긴 것입니다.**
> 이 절의 예전 판은 *"그 어느 숫자와도 대응하지 않습니다"* 라고 적었는데 **틀렸습니다.**
> 두 도식이 그걸 드러냅니다, 둘 다 세 단계에 화살표 하나, 그리고 **⟹ 가속**으로 끝납니다.
> 이미 같은 모양이었고 이름만 떨어져 있었습니다. 그러니 이건 **한 층을 두 번 말한 것**이지
> 두 번째 층이 아닙니다. 4대 엔진 · 4축 게이트 · 6축 검증은 여전히 다른 숫자이고 여기 대응하지 않습니다.

**모루 위에서 셋, 그리고 각각이 3단 공정의 한 단계입니다:**

| 대장장이의 말 | 단계 | 여기서는 무슨 뜻인가 | 명령 |
|---|---|---|---|
| **벼림(Forge)** | ① **영혼 심기** | 날것의 프로젝트를 하네스로 형태 잡고, **설계 전에** '무엇을 성공이라 부르나 / 절대 안 하는 것'을 정함 | `install-wizard` · `deep-clarify` · 마커의 필수 `①영혼` 줄(커밋에서 강제) |
| **담금질(Quench)** | ② **병렬 탈상관 가속** | 급랭 · 탈상관된 각도로 여러 시도를 병렬로. 빠르게 단단해지고, 🟥 **그 대가로 취성이 생깁니다.** 강철에 담금질이 하는 일이 정확히 그것입니다 | `auto-decorrelation` · `agent-composer` · `meta-prompt-builder` · 타계열 사이드카(codex · agy · gemini) · 격리 워크트리 레인 |
| **뜨임(Temper)** | ③ **6축 태우기** | 재가열해 경도는 지키고 취성만 빼냄 · **공격하는 패스들이 실제로 속하는 자리** | `steel-quench` · `phantom-quench` · `sim-conductor` · `prompt-regression` · `verify-bidirectional` · `fh-meta:challenger` · 되돌림 프로브 · `templates/temper_check.sh` |

> ⚠️ **스킬 이름은 예전 독법을 담은 채 그대로 둡니다.** `steel-quench` · `phantom-quench` 는
> 이름이 *담금질*이지만, 하는 일, 단단해진 자산을 공격해 취성을 드러내는 것, 은 **뜨임**입니다.
...
> 어긋났습니다. 스킬을 더 많이 쓰는 것 자체도 목표가 아닙니다. 작업에 실제로 필요한
> 것을 쓰는 게 목표입니다.
...
서브에이전트로 디스패치됩니다. 세션 모델은 건드리지 않습니다. **FH는 절대 사용자의 세션
...
> - **v1.0 · 방법론** · [Zenodo](https://zenodo.org/records/20397566) (DOI 10.5281/zenodo.20397566). 2층 설계, 6축 프레임워크, 4-에이전트 오케스트레이션 · 복리 루프를 실증 증거와 함께.
