...
**然后输入 `你好`** —— 用你习惯的语言问候即可（`hi`、`안녕`、`こんにちは`）。**它会用你问候的
那种语言回复**，所以你不必为了用它而切换到英文。会出现一个带编号的菜单，之后由工具引导你：
选一个入口，回答几个问题，它会替你运行安装向导。
...
> ✅ 然后 **打一句招呼** —— `你好`、`hi`、`안녕`、`こんにちは`，用你习惯的语言就行，它会用那种
> 语言回复。🐿️ 门菜单是在你 *打出* 招呼时出现的，光是启动不会出现。
