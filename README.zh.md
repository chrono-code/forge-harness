...
独立的、特化的框架。这就是它所朝向的目标。

### 你实际得到什么 —— 五重身份

这不是五个模块。它们是 **技能自然聚拢成的形状** —— 是给一个早已存在的东西命名，它散布在各个技能
与 agent 之中，而不是叠加在它们之上。
...
成熟度按身份逐项跟踪，用一把四级刻度 —— `ideal → partial → RC（在实验室里立起来了）→
REALIZED（走到外面去了）` —— 而这些等级刻意只存放在一个地方：
[`ship_readiness_gate.md`](knowledge/shared/harness-core/ship_readiness_gate.md)。它们 **没有** 被
复制到这里；同一个等级放进两个文件，总会有一个先腐坏，这一点本仓库在自己身上测到过不止一次。
...
- **它搭上前沿，而不是给前沿打补丁。** FH 跨家族派发（Claude、Codex、Gemini、本地）—— 但重点
  *不是* 去糊住每个模型的弱点，因为随着模型变强，那套脚手架会死掉。它是共同演化 (co-evolution)：
  底座 (substrate) 现在原生就能做到的就卸掉，它接下来推出的就吸收进来。去相关 (decorrelation) 是
  当下的信任杠杆；跨家族面板胜过单一模型的上限。
...
**四大引擎。** 每一个都是上面某个身份所站立的地基。门禁表早就机械地承载着这四者；给它们命名是
识别，不是发明。
...
`judgment-circuit` 是最容易被误读的一个，所以直说：它 **不是一份身份宣言**。也不要把它简写成英文
里的 "soul"（或中文的"灵魂"）—— 那个词读起来是 *人设 (persona)*，而这个引擎背后那 105 次运行的
测量，最大的一项发现恰恰是：**一份人设宣言并不是一套判断坐标系** ——"你是一个 ~"在弱层级上测出来
是 *净损失*，把它移除反而回收了 **+0.67**。一个词的改名，会把测量刚刚分开的两样东西重新焊回去。
判断坐标系也不是一次坐下就能建成的 —— FH 交给一个新框架的是一份 **种子草稿**，随着那个框架被真正
用起来而逐步填满。
...
② 中段做去相关，用来加速  选定轴，然后一次性打过去。要「挑」轴，不要「乘」轴 ——
                        并行本身没有方向，方向由判断坐标系给出

③ 最后在四条轴上烧一遍    也就是下面那四条轴。对抗审阅只是其中一条，不是全部
```

**四条验证轴** —— 所谓"我们审过了"，往往到头来只做了第一条：

| 轴 | 这条轴响的时候，是什么错了 | 典型手段 |
|---|---|---|
| **ⓐ 不同家族** | **实现** 错了 | 跨家族对抗审阅（`auto-decorrelation`） |
| **ⓑ 首次真实使用** | **你测量的方式** 错了 | 拿一个真实目标真跑一次 |
| **ⓒ 记录接地** | **主张** 错了 | 由一次隔离的审计，重新测量文档里的数字与引用 |
| **ⓓ 撤回并观察** | **锚** 错了 | 把接线删掉，看对应的那条 lane 是不是真的变红 |

还有第五条轴 —— **立场 (standpoint)** —— 当一次变更跨进另一个框架时适用：从 *目标方* 的仓库与
规则去跑这份 diff，而不是凭你自己对它们的理解
（[`field_verdict_crossfamily_gate.md §7`](knowledge/shared/harness-core/field_verdict_crossfamily_gate.md)）。

你不必每次都把四条轴跑满；你挑那些对得上你此刻真正暴露的失败模式的轴。这条成本边界是刻意的 ——
一行小修一条都不配，判定/门禁类的变更配得上 ⓐ，而不可逆的面配得上更多。
