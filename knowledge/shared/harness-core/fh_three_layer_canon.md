...
> d=되돌림). 즉 **산문 정본은 6축인데 기계는 4+1축이고, 문자가 어긋난다.** ⚠️ 「4」가 아니라
> **「4+1」**이다 — `axes-run` 네 글자에 더해 ⓑ입장이 **`standpoint:` 자기 필드로 이미 기계화돼
> 있다**(§1-a-2 와 CLAUDE.md 상주줄이 둘 다 «4+1» 로 못박는다). 「4」로 줄이면 그 사실이 사라져
> **이 표가 고치려는 것과 같은 클래스의 새 드리프트를 이 파일 안에 만든다**(입장 축 적발). 새 배열로 마커를 쓰면
