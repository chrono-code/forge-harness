...
ⓓ 3자 대면             ✅ 기록할 자리가 생겼다 (`ⓓ=`)
```
🟥 **표기법이 배열을 선언한다** — ASCII 키 = 옛 4축 · 기호 키 = 현 6축. 이렇게 가른 이유는 미관이
아니라 실측이다: 두 배열은 **같은 글자가 다른 축을 가리킨다**(옛 `b`=첫실사용 → 지금 **ⓔ**,
옛 `d`=되돌림 → 지금 **ⓕ**). 같은 글자를 쓰면서 의미만 바꾸면 훅이 **조용히 다른 축으로 읽으며
오류를 내지 않는다** — `not found ≠ 0` 의 형제다.
