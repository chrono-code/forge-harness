...
| **Measurement-reps** (2026-08-29, 운영자 지적에서 신설) | **Mechanical grep**: 그 하네스의 기록(마커·판정·신호·릴리스 노트)에서 `reps` 표기를 뽑아 **바 미달(1~2)** 인 것을 찾는다. **Judged**: 그 측정이 떠받치는 결론이 «하중을 지나» — 판정 · 등급 · 교리 변경 · 비가역 처방 중 하나인가. 아니면 계상하지 않는다(계기 확인용 1회는 올릴 값이 없고, 전부 올리면 소음이 된다). **처방은 언제나 「sim 으로 올리자」 한 줄** — 자동 실행이 아니라 랭크. | 🟥 지금까지의 기본값: 「reps=1, 바 미달」이라 **적고 넘어간다**. 라벨은 측정이 아니다. 실측 2026-08-29 — 같은 세션에서 두 번 그렇게 적었고 **두 번 다 운영자가 밀어서** reps=3 으로 올라갔으며, 올려보니 **결정적**이었다(ARM 3/3 · CONTROL 0/3). 즉 그 라벨들은 「모른다」가 아니라 **「안 재봤다」**였다. ⚠️ 이 렌즈는 FH 자산에만 걸리는 규율이 아니다 — 필드 하네스가 자기 개선을 기록할 때 같은 자리가 생기고, 오히려 그쪽이 표본이 더 귀하다 |
