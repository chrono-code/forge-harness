...
| 29119-11 테스트 오라클 문제 | 기대값을 정할 수 없을 때의 오라클 | **known-pair**(양성/음성 컨트롤이 같은 실행에) · 결과 전 채점기 고정 · «미측정 ≠ 0」 · UNCALIBRATED 라벨 | `measurement-integrity-checklist.md §Instrument-Calibration` · `CLAUDE.md §Instrument Calibration` | ✅ 기제 / ❌ **오라클 유형** 을 기록하는 채널 없음 → §4 M1 |
...
|---|---|---|---|---|
| **M1** | **오라클 유형 채널** | 레인/마커/TC 에 `oracle:` 값 — `known-pair` · `metamorphic` · `back-to-back` · `A/B` · `human` · `none(미측정)` — 닫힌 enum, 훅은 형식만 | 29119-11 의 핵심이 «오라클 문제」다. 지금은 «컨트롤이 살아 있다」까지만 적고 **어떤 종류의 오라클**인지는 안 적는다 — 외부 독자가 첫 번째로 묻는 것 | 낮음(옵셔널 필드 + enum). 값의 진위는 안 본다(형식만) |
