...
🟥 **그리고 두 결함이 서로를 가려주고 있었다.** `positive ARM 5/6 PASS` 는 운반체를 읽어서가
아니라 **`claude -p` 가 상속된 stdin 으로 받은 정답키** 때문이었다(같은 날 1차 철회). 한쪽만
고쳤으면 다른 쪽이 여전히 그럴듯한 숫자를 냈을 것이다.

⇒ **`DELIVERY` 는 이 저장소에서 «한 번도 측정된 적이 없다».** 아래 표의 네 숫자 전부 무효이고,
