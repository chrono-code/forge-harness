...
5. 회수 후에는 **내용 검증을 해라** — 파일이 있다 ≠ 내 최신 편집이다. 마지막에 넣은 수리가
   들어있는지 토큰으로 확인해라(위 사례는 cross-family 수리 3종을 grep 으로 확인했다).

⚠️ **근본 해결은 아직 없다.** 게이트가 스스로 «세션당 worktree» 를 권하는데, **FH 자산은
워크트리에서 커밋이 안 된다** — `tracks/` 가 gitignored 라 안 따라오고 Axis 2+3 마커와 Axis 4
매니페스트가 **구조적으로 부재**하기 때문이다(`CLAUDE.md §Agent Dispatch Operation`, 2026-08-05
양팔 실측). 그래서 「격리하라」와 「FH 자산은 격리에서 못 커밋한다」가 **정면으로 충돌**한다.
이 모순은 이 문서가 풀 수 있는 게 아니다 — **명시된 미해결**로 남긴다.
