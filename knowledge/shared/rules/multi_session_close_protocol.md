...
반환한다. 2026-08-09: 5곳에 보내고 카드에 「5곳 전부 통지」라고 적었는데 **2건이 도달하지 않았다.**
...
소켓이 살아있으면 **프로세스**가 살아있는 것이지 세션이 활성인 게 아니다. 그런데 그 여분을
노이즈로 버리면 안 된다 — 2026-08-09 실측상 여분은 **Completed 로 보이지만 마감을 끝까지 안 해서
프로세스가 남은** 세션이었다.
```
alive + Working    → 활성 peer. 접기 전에 물어라
alive + Completed  → 그 세션 마감이 안 끝났다. 이 상태를 보여주는 곳이 달리 없다
```
