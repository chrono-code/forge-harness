...
| **주어가 빠져 있다** | 🔴 **미구축** | 한국어 주어 생략은 정상 문법이라 «생략»과 «누락»을 가르려면 문맥이 필요하다. 패턴만으로는 못 한다 |
| **화면이 너무 비어 보인다** | 🔴 **미구축** | gate.py 는 «밖으로 나갔나»를 보지 «안이 비었나»를 안 본다. 시각 밀도는 렌더를 봐야 하고 이 머신엔 pptx 렌더러가 없다 |
