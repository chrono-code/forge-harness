...
20곳을 삼켰는데 빌드는 성공했고, S8 을 빼자 S9 가 성립 불가가 됐는데 렌더 28장을 눈으로 다
봐도 안 잡혔으며, 도해에 구워진 텍스트는 XML 을 읽는 눈에 구조적으로 안 보였다.
...
| 운영자 예시 | 지금 | 무엇이 보나 |
|---|---|---|
| **위치가 어긋나 있다** | 🟢 덮인다 | `ooxml/gate.py` ⑥ 화면 밖 도형(실사고: 26p 가 우측으로 1,854,400 EMU 이탈) |
...
| **화면이 너무 비어 보인다** | 🔴 **미구축** | gate.py 는 «밖으로 나갔나»를 보지 «안이 비었나»를 안 본다. ⚠️ 렌더 경로 재확인(2026-08-29): `pdftoppm` 은 있으나 **`soffice` 가 없어 pptx→PDF 가 끊긴다** — 어느 기록이 「LibreOffice 를 깔았다」고 적고 있어도 **지금 이 머신에는 없다**(기록은 당시 상태이지 현재 상태가 아니다). 기준선 형태는 정해져 있다: **레퍼런스 덱을 실측해 장 유형별 화면 자수 기준선을 세우고 미달 장을 센다** — 값은 발표마다 다시 재야 한다 |
| **흐름이 매끄럽지 않다** | 🔴 **미구축** | L8 은 «지시 대상이 해소되나»를 볼 뿐 «논지가 서나»를 못 본다 |
...
이유도 같다: FH 실측(105런)에서 **영혼을 먼저 심는 것이 결정적**이었고, 선언을 빼면 약한 모델의
성능이 눈에 띄게 갈렸다. 그래서 ①은 요구사항 수집이 아니라 **같이 세우는 것**이다.
